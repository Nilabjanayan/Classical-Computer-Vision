...
def hough_lines_vote_acc(edge_img, rho_res=1, thetas= np.arange(0,180)):
    ...
    ### TODO: YOUR CODE HERE ###
    h,w = edge_img.shape
    a = int(np.sqrt(h**2 + w**2))
    rhos = np.arange(0,a,rho_res)
    H = np.zeros([int(len(rhos)),len(thetas)])
    thetas1=np.deg2rad(thetas)
    for i in range(h):
        for j in range(w):
            if (edge_img[i,j]==255):
                for theta in range(len(thetas)):
                    rho = int(j*np.cos(thetas1[theta]) + i*np.sin(thetas1[theta]))
                    H[rho , np.int(np.round(np.rad2deg(thetas1[theta])))] +=1
...
def hough_circles_vote_acc(edge_img, radius):
    ...
    for i in range(h):
        for j in range(w):
            if edge_img[i,j] == 255:
                for theta in range(len(thetas)):
                    a=int(i + radius*np.cos(thetas[theta]))
                    b=int(j + radius*np.sin(thetas[theta]))
